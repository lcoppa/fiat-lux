...
def main():
    ...
    #
    # This example demonstrates another feature. By default, all resource types
    # implement their default values as defined with the resource definition. A
    # SNVT_switch type, for example, is defined to default to value,
    # state = 0, 0. The constructor supports overriding the defaults' defaults
    # with application- specific default values, demonstrated here.
    # Also note that all Python-facing datapoint values are 'scaled' as and if
    # necessary. The Python script never 'sees' so-called 'raw' data.
    # Initializing the value of my_switch_input to 15 means to set the level to
    # 15% (the raw value equivalent is 30).
    #
