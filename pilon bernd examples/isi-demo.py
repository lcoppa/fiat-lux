...
"""A simple pilon ISI demo application.
    ...
    python3 isi-demo.py --device //10.0.2.123:1628/uc --nvd .isi-demo-A-nvd \
       --log isi-demo-A

    python3 isi-demo.py --device //10.0.2.123:1632/uc --nvd .isi-demo-B-nvd \
       --log isi-demo-B

    (Obviously, you'd need to adjust IP address and port in the device command
    to match your local environment and IP-852 configuration server.)
...
def main():
    ...
    parser.add_argument(
        '-d', '--device',
        required=True,
        help='The device URI, e.g. //10.0.1.12:1628/uc'
    )
    parser.add_argument(
        '-n', '--nvd',
